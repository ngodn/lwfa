...
mod tests {
    ...
    #[test]
    fn an_existing_argon2_05_account_still_authenticates_after_upgrade() {
        // Generated once with argon2 0.5.3 and its default parameters, using
        // public fixture password/salt. This is never regenerated by the test.
        let legacy = "$argon2id$v=19$m=19456,t=2,p=1$bHdmYS11cGdyYWRlLWZpeHR1cmU$okhbXNGFF2zxocMh7Ymt4qtgzObtLoHVs6tuMulsJvQ";
        let accounts = Accounts::in_memory().unwrap();
        accounts.db.execute(
            "INSERT INTO users (name, hash, mode, allowed_apps) VALUES (?1, ?2, 'view', '[]')",
            params!["legacy", legacy],
        ).unwrap();
        let found = accounts.authenticate("legacy account test password").expect("legacy PHC must remain valid");
        assert_eq!(found.name, "legacy");
        assert_eq!(found.permissions, view_only());
        assert!(accounts.authenticate("wrong password").is_none());
        let current = hash_password("new account password").unwrap();
        assert!(current.starts_with("$argon2id$v=19$m=19456,t=2,p=1$"), "password parameters changed: {current}");
        assert!(verify("new account password", &current));
    }
