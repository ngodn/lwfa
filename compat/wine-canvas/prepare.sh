...
apply_patch() {
    ...
    patch --batch -Np1 < "$path" || status=$?
    path="${path//\/\//\/}"
    if (( status != 0 )); then
        printf '%s\t%s\n' "$status" "$path" >> "$record"
        if (( status != 1 )); then return "$status"; fi
        case "$path" in
            ../patches/wine-hotfixes/wine-staging/winex11-Fixed-scancodes/0001-winecfg-Move-input-config-options-to-a-dedicated-tab.patch|\
            ../patches/wine-hotfixes/wine-staging/d3drm-starwars/d3drm-starwars-Werrror-fixup.patch|\
            ../patches/wine-hotfixes/wine-staging/mmsystem.dll16-MIDIHDR_Refcount/mmsystem.dll16-MIDIHDR_Refcount-Werror-fix.patch|\
            ../patches/wine-hotfixes/pending/ws2_32-validate-connect-address.patch|\
            ../patches/proton-ds5-haptic/0022-winepulse-bound-format-probes-during-ds5-hotplug.patch|\
            ../patches/proton-ds5-haptic/0109-hid-mmdevapi-optionally-use-windows-sony-controller-names.patch) ;;
            *) echo "Unexpected rejected patch: $path" >&2; return "$status" ;;
        esac
    fi
    return 0
}
apply_all_in_dir() {
    local dir="$1" patch
    for patch in "$dir"/*.patch; do apply_patch "$patch"; done
}
(cd wineopenxr; apply_all_in_dir ../patches/wineopenxr)
cd wine
patch --batch -Rp1 < "$configure_patch"
