...
// Optional: LWFA_TEST_MONITOR_SIZE=WxH tests an explicitly configured reserve;
// LWFA_TEST_BROWSER=1 checks a built-shell refresh preserves a chosen column width.
...
const configured = process.env.LWFA_TEST_MONITOR_SIZE?.match(/^(\d+)x(\d+)$/);
assert(!process.env.LWFA_TEST_MONITOR_SIZE || configured, 'LWFA_TEST_MONITOR_SIZE must be WxH');
const expected = configured ? { width: Number(configured[1]), height: Number(configured[2]) } : viewport;
...
const results = { startedAt: new Date().toISOString(), viewport, expected, phases: [] };
...
async function connect() {
  ...
  socket.addEventListener('message', ({ data }) => {
    ...
    if (message.type === 'hello') { hello = true; windows.clear(); for (const window of message.windows) windows.set(window.id, window); }
...
`);
  ...
  if (!configured) assert.deepEqual(results.before.xwaylandPids, [], 'Automatic Xwayland must await the first valid browser viewport');
  ...
  assert.deepEqual({ width: application.width, height: application.height }, expected, 'Application must observe the selected monitor, without multiplying by browser DPR');
  ...
  results.phases.push({ phase: 'first-viewport', monitor: monitor(application.display), xwaylandPids: serverPids, window: id });
  for (const [name, size] of [['page-refresh', viewport], ['changed-viewport', { width: 1490, height: 910 }]]) {
    ...
    assert.deepEqual(observed.monitor, expected, 'Reconnect and browser changes preserve the active display mode');
  ...
  if (process.env.LWFA_TEST_BROWSER === '1') {
    ...
    try {
      ...
      results.browser = { beforeChoice, before, after, reloadGeometry, declarations, passed: false };
